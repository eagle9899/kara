@php
$meta_title = $subcategory->sub_category . ','. $category->name ;
...
$meta_desc = $subcategory->sub_category .','. $category->name . $desc;
$title = $subcategory->sub_category . ' | '.$category->name . 's Kara' .$title;
@endphp
...
<div class="col-md-6 p-5 mt-lg-5">
    ...
    <nav aria-label="breadcrumb animated fadeIn">
        <ol class="breadcrumb text-uppercase">
            ...
            <li class="breadcrumb-item"> <a href="{{ route('category',$category->slugs) }}">{{ $category->name }}</a>
            </li>
            <li class="breadcrumb-item text-body active" aria-current="page">{{ $subcategory->sub_category }}</li>
        </ol>
    </nav>
</div>
@endsection
    ...
    <div class="container">
        ...
        <div>
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="property-item rounded overflow-hidden">
                        <div class="position-relative overflow-hidden">
                            ...
                            <div id="carouselExampleInterval{{ $item->id }}" class="carousel slide"
                                ...
                                <div class="carousel-inner">
                                    ...
                                    <div class="carousel-item {{ $loop->first ?'active' :'' }}"
                                        ...
                                        <a
                                            href="{{ route('category_by_subcategory_detail',$item->rCategory->slugs.'/'. $item->rSubcategory->slug.'/'. $item->slug) }}">
                            ...
                            <div
                                class="bg-primary text-capitalize rounded text-white position-absolute start-0 top-0 m-4 py-1 px-3">
                                {{ $item->type }}</div>
                            <a href="" type="button"
                                class="position-absolute text-white end-0 top-0 m-4 py-1 px-3 btn btn-ouline-success favorite border"><i
                                    class="bi bi-heart"></i></a>
                            {{-- <div class="bg-primary rounded text-white position-absolute end-0 top-0 m-4p y-1 px-3">
                                                $item->rCategory->name </div> --}}
                            <div
                                class="bg-white rounded-top text-primary position-absolute start-0 bottom-0 mx-4 pt-1 px-3">
                                {{ $item->rSubcategory->sub_category }}
                            </div>
                        </div>
                        <div class="p-4 pb-0">
                            <h5 class="text-primary mb-3">{{ $item->currency }} {{ $item->rate }}</h5>
                            <a class="d-block h5 mb-2 text-capitalize post-short-title"
                                href="{{ route('category_by_subcategory_detail',$item->rCategory->slugs.'/'. $item->rSubcategory->slug.'/'. $item->slug) }}">{{ substr($item->title, 0, 28,) }}
                                ...</a>
                        ...
                        <div class="d-flex border-top">
                            ...
                            <small class="flex-fill text-center py-2"><i
                                    class="fa fa-shower text-primary me-2"></i>{{ $item->bathrooms }}
                                Bath</small>
                        </div>
                    </div>
                </div>
            ...
            <div class="row g-4 justify-content-center mt-3">
                <div class="col-lg-6">
                    <div class="justify-content-cventer">{{ $posts->links() }}</div>
